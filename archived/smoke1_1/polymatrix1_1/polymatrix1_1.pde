void setup(){
  ...
  smooth();
  //background(22,22,29);
 stroke(random(100,150),random(130,170),random(70,150),random(170));
  background(255);
}
float h=540;
float w=960;
...
void draw(){
  ...
  if(strokeW>0.005){
    ...
    if(random(1000)>=998){
        drawEl(xr,yr);}
  xr=randomDistance(x,strokeW);
  yr=randomDistance(y,strokeW);
 }
 else{
   if(random(1000)>=900){
      i=0;
...
public float randomDistance(float x, float i){
  float xrandom;
  float dist=25;
  float k=(sqrt(i));
 
  xrandom=x+random(-dist,dist)*k;
  return xrandom;
}
